	...
	public class DirectionalShadowMapper extends ShadowMapperBase
	{
		...
		*/
		...
		override protected function updateDepthProjection(viewCamera:Camera3D):void
		{
			...
			//_overallDepthCamera.lens.far = 10000;
		...
		protected function updateProjectionFromFrustumCorners(viewCamera:Camera3D, corners:Vector.<Number>, matrix:Matrix3D):void
		{
			...
			op = new Vector3D(viewCamera.x,viewCamera.y + 10000,viewCamera.z - 10000);
			_overallDepthCamera.position= op;
			...
			minX = 10000;//-16200;//_localFrustum[0];
			maxX = 10000;
			minY = 10000;//-13148;//_localFrustum[1];
			maxY = 10000;
			_maxZ = 10000;//26000;//_localFrustum[2];
			_minZ = 1;
			...
			_minZ = 1;
			//if (Main.instance.game.fpsLabel != null)
			//{
				//if (Main.instance.game != null)
				//{
					//Main.instance.game.fpsLabel.text = minX + "	" + minY + "	" + maxX + "	" + maxY + "	" + _maxZ;
				//}
			//}
			
			var w:Number = maxX - minX;//26200
			var h:Number = maxY - minY;//23148
			var d:Number = 1/(_maxZ - _minZ);//0.000038610038610039
			
			if (minX < 0)
				minX -= _snap; // because int() rounds up for < 0
			if (minY < 0)
				minY -= _snap;//-128
			minX = int(minX/_snap)*_snap;//-128
			minY = int(minY/_snap)*_snap;//-128
			
			var snap2:Number = 2*_snap;//256
			w = int(w/snap2 + 2)*snap2;//26112
			h = int(h/snap2 + 2)*snap2;//23040
